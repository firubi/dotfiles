...
### Setting up Hyprland ###
Firstly, I'd recommend tabooing sway, kanshi and grim in YaST (as they are recommended packages). Then install packages: `sudo zypper in --no-recommends hyprland waybar swww`. For the rofi menu to work for logout, you need to, in powermenu.sh, add 
```elif [[ "$DESKTOP_SESSION" == 'hyprland' ]]; then				hyprctl dispatch exit```
